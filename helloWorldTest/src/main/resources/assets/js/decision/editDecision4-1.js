...
editDecision4.init =function(){
	...
	//input输入框操作
	editDecision4.inputOption();
}
//初始化调用
$(document).ready(editDecision4.init);
//input输入框操作
editDecision4.inputOption = function(){
    ...
    $.ajax({
        ...
        success: function(result) {//回调函数，result，返回值
            if(null != result && "" != result && "undefined" != typeof(result)){
                ...
                $("#min").text(min);
                $("#max").text(max);
                if((min==data.passStart&&max==data.refuseEnd)||(max==data.passStart&&min==data.refuseEnd)){
                    if(min==data.passStart){
                        editDecision4.range=[
                            {'dataId':'0','name':min},
                            {'dataId':'1','name':max},
                        ];
                    ...
                    $("#passStart").val(data.passStart);
                    $("#passEnd").val(data.passEnd);
                    $("#reviewStart").val(data.reviewStart);
                    $("#reviewEnd").val(data.reviewEnd);
                    $("#refuseStart").val(data.refuseStart);
                    $("#refuseEnd").val(data.refuseEnd);
                    $('.config_number').html(data.refuseEnd);
                }else{
                    editDecision4.range=[
                        {'dataId':'0','name':min},
                        {'dataId':'1','name':max},
