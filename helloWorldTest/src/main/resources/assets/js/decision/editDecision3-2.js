//页面模块名称
var editDecision3 = {};
editDecision3.ruleList=[];
editDecision3.ruleTree=[];
//初始化
editDecision3.init =function(){
    //获取规则数据
    editDecision3.findAllRule();
    //获取决策树数据
    editDecision3.findTreeData();
    treePublic.beginDecisionTree({
        dataMsg:editDecision3.ruleTree,
        //下拉菜单数据
        dropMenu:editDecision3.ruleList
    });
...
editDecision3.findAllRule=function (){
    $.ajax({
...
editDecision3.save=function(){
    ...
    $.ajax({
        ...
        success: function(result) {//回调函数，result，返回值
            if(result == 1){
                window.location.href = sceneStep3UpdateDecisionConfig+id;
            }else{
                aide.alert("新增失败");
            }
        },
...
editDecision3.findTreeData=function (){
    $.ajax({
