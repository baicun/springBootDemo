//页面模块名称
var editDecision2 = {};
editDecision2.num = 0; //模拟计数不使用可以删除
editDecision2.optionData = []; //模拟选中数据不使用可以删除
editDecision2.filedByUsedArr = [];//存储选中的规则所关联到的字段信息
//简单模拟数据
editDecision2.data = ['姓名','年龄','城市','工作地','银行卡预留手机号','姓名1','年龄1','城市1','工作地1','银行卡预留手机号1','姓名2','年龄2',]
//初始化
editDecision2.init =function(){
    editDecision2.findFieldList();
	//生成页面内容模版
	editDecision2.createHtml();
	//获取选中的字段
    editDecision2.getSceneSelectedField();
}
//初始化调用
$(document).ready(editDecision2.init);
//生成页面内容模版
editDecision2.createHtml = function(){
	$('#config_box2').append(`
		<div class="config_option fl">
			<div class="search_text radius">
				<input type="text" placeholder="请输入字段名称/英文名称">
	...
	`);
	//字段选择列表
	for(var i=0;i<editDecision2.data.length;i++){
		$('#config_option_list').append(`
			<li>
				<a href="javaScript:;">
					<span data-id="0" data-dataid="${editDecision2.data[i].id}"></span>
					<strong>${editDecision2.data[i].name}</strong>
				</a>
			</li>
		`)
	}
	//字段选择列表-如果数据的长度大于11出现滚动条
	if(editDecision2.data.length>11){
...
//字段选择-选择字段点击操作
editDecision2.optionFields = function(){
	//字段选择按钮点击
	$('.config_option h2 a').off('click').on('click',function(){
		var dataId = $(this).attr('data-id');
		editDecision2.optionData=[];
		if(dataId == 0){
			$('#config_option_list span').addClass('active');
			$('#config_option_list span').attr('data-id',1);
			$(this).addClass('active');
			$(this).attr('data-id',1);
			//editDecision2.num = $('#config_option_list span').size();
			editDecision2.dynGetRules(editDecision2.getSelectedFieldId);
			editDecision2.associationRules(editDecision2.optionData);//生成关联规则列表
            $('.config_content_num span').html(editDecision2.num);
            $('#no_info').hide();
		}else{
	...
	$('#config_option_list span').off('click').on('click',function(){
		...
		if(dataId == 0){
			...
			for(var i=0;i<$('#config_option_list span').size();i++){
				if($('#config_option_list span').eq(i).attr('data-id')==1){
					editDecision2.optionData.push($('#config_option_list strong').eq(i).html());
				}
			}
			//editDecision2.num++;
			// $('.config_content_num span').html(editDecision2.num);
			// if(editDecision2.num!=0){
			// 	$('#no_info').hide();
			// }else{
			// 	$('#no_info').show();
			// }
...
editDecision2.getSelectedFieldId=function(){
	var fields="";
...
//生成关联规则列表
editDecision2.associationRules = function(data){
	$('#config_rule_list').html('');
...
//获取字段列表信息
editDecision2.findFieldList=function  () {
    $.ajax({
...
//获取和字段相关联的规则信息
editDecision2.dynGetRules=function (fieldArray) {
    $.ajax({
        ...
        success: function(data) {//回调函数，result，返回值\
            ...
            editDecision2.num=data.length;
            editDecision2.optionData = data;
        }
    })
}
//保存字段数据
editDecision2.saveStep2=function(){
    ...
    $.ajax({
        ...
        success: function(data) {//回调函数，result，返回值\
            if(data == 1){
                window.location.href = sceneStep2UpdateRuleConfig+$("#sceneId").val();
            }else{
                aide.alert("至少选择一个规则");
            }
        },
...
//获取已选择规则已使用的字段
editDecision2.getFiledByUsed=function (){
    $.ajax({
        ...
        async : false,
        success: function(data) {//回调函数，result，返回值\
            editDecision2.filedByUsedArr = data;
        }
    })
}

//获取场景选择的字段
editDecision2.getSceneSelectedField=function (){
    //查询场景字段
    $.ajax({
        ...
        success: function(data) {//回调函数，result，返回值\
            if(data.length > 0){
                ...
                editDecision2.dynGetRules(fields);
                editDecision2.associationRules(editDecision2.optionData);//生成关联规则列表
                $('.config_content_num span').html(editDecision2.num);
                if(editDecision2.num==0){
                    $('#no_info').show();
