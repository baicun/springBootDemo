...
rulePublic.ruleMap=new Map();//记录规则基本信息以便展示
...
rulePublic.htmlTemplate = `<div class="logic_set_list set_number">
	...
	<div class="logic_content">
		...
		<div class="strategy_setUp clear">
			...
			<p class="strategy_setUp_btn fl" data-id="">
				<a href="javaScript:;" data-id="0"><span>通过</span></a>
				<a href="javaScript:;" data-id="1"><span>拒绝</span></a>
				<a href="javaScript:;" data-id="2"><span>人工复核</span></a>
			</p>
			<p class="dec_set_error">请选择策略设置</p>
		</div>
...
rulePublic.strategySetup = function(){
	$('.strategy_setUp_btn a').off('click').on('click',function(){
        ...
        $(this).parent().parent().find('.dec_set_error').hide();
	})
};
//运算设置
rulePublic.operationSet = function(opt){
	...
	var str = '';
	for(var i=0;i<rulePublic.arrText.length;i++){
		if(rulePublic.arrText[i].type == 5){
			str = `<div class="setUp_type1">
						...
						<div class="setUp_text fl">
							<input type="text" placeholder="请输入"  name="include_text" id="include_text_${rulePublic.index++}"/>
							<i class="setUp_error">输入错误</i>
						</div>
					</div>`
		}else if(rulePublic.arrText[i].type == 1){
			str = `<div class="setUp_type2">
						<div class="setUp_text fl">
							<input type="text" class="setUp_text_input" placeholder="请输入" name="number" id="number_${rulePublic.index++}"/>
							<span>(含)</span>
							<strong>-</strong>
							<i class="setUp_error">输入错误</i>
						</div>
            ...
            str = `<div class="setUp_type2">
						<div class="setUp_text fl">
							<input type="text" class="setUp_text_input" placeholder="请输入" name="double" id="double_${rulePublic.index++}"/>
							<span>(含)</span>
							<strong>-</strong>
							<i class="setUp_error">输入错误</i>
						</div>
		...
		}else if(rulePublic.arrText[i].type ==4){
		...
		$('#'+opt.listId).find('tbody').append(`
			<tr data-type="${rulePublic.arrText[i].type}" data-name="${rulePublic.arrText[i].name}" data-id="${rulePublic.arrText[i].uid}">
				<td class="setUp_content_tab_td1">${rulePublic.arrText[i].name}</td>
				<td class="setUp_content_tab_td2">${rulePublic.arrText[i].typeName}</td>
	...
	for(var i=0;i<rulePublic.newDataArr.length;i++){
        ...
        $('#'+opt.listId).find('.down_btn2').eq(i).find('input').attr("data-code",rulePublic.newDataArr[i][0].code);
	}
	//下拉菜单点击
...
rulePublic.downMenu = function(opt){
	...
	if(opt.dataType==1){
		...
		for(var j=0;j<rulePublic.newDataArr[opt.dataValue].length;j++){
			str+=`<li data-id="${j}" data-code="${rulePublic.newDataArr[opt.dataValue][j].code}"><a href="javaScript:;">${rulePublic.newDataArr[opt.dataValue][j].name}</a></li>`
		}
	}
	$('.body_menu_list').html('');
	...
	$('.body_menu_list li').off('click').on('click',function(){
		...
		$('.down_menu_btn').attr('data-id',0);
		if(opt.dataType==3){
            $('#'+opt.id).find('.down_menu_btn').eq(opt.dataId).find('input').attr('data-code',$(this).attr('data-code'));
		}
	});
